...
const getDocumentTree = async () => {
  try {
    // Get all documents
    const result = await query(`
      SELECT * FROM documents ORDER BY parent_id NULLS FIRST, created_at ASC
    `);
...
const createDocument = async (title, parentId = null) => {
  try {
    const result = await query(`
      INSERT INTO documents (title, parent_id, updated_at)
...
// Update a document
const updateDocument = async (id, title) => {
  try {
    const result = await query(`
      UPDATE documents
...
const createDocumentBlock = async (documentId, type, content) => {
  try {
    const result = await query(`
      INSERT INTO document_blocks (document_id, type, content, updated_at)
...
const updateDocumentBlock = async (id, type, content) => {
  try {
    const result = await query(`
  ...
  try {
    ...
    const result = await query(`
      WITH RECURSIVE document_path AS (
        SELECT id, title, parent_id, ARRAY[id] AS path, ARRAY[title]::varchar[] AS titles
        FROM documents
        WHERE id = $1
