...
router.post('/', isAdmin, async (req, res, next) => {
  try {
    ...
    const document = await documentService.createDocument(title, parentId);
...
router.put('/:id', isAdmin, async (req, res, next) => {
  try {
    ...
    const document = await documentService.updateDocument(id, title);
...
router.put('/:id/move', isAdmin, async (req, res, next) => {
  try {
    ...
    const document = await documentService.moveDocument(id, parentId);
...
router.post('/:id/blocks', isAdmin, async (req, res, next) => {
  try {
    ...
    const block = await documentService.createDocumentBlock(id, type, content);
...
router.put('/blocks/:blockId', isAdmin, async (req, res, next) => {
  try {
    ...
    const { type, content } = req.body;
    const block = await documentService.updateDocumentBlock(blockId, type, content);
