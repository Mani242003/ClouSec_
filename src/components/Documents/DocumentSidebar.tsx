...
const DocumentSidebar: React.FC<DocumentSidebarProps> = ({ onSelectDocument, selectedDocumentId }) => {
  ...
  const toggleNode = (nodeId: number, document?: Document) => {
    const isCurrentlyExpanded = expandedNodes[nodeId] || false;
    ...
    // If we're expanding a node and it has children, automatically select the first child
    if (!isCurrentlyExpanded && document?.children?.length) {
      onSelectDocument(document.children[0]);
    }
  };
    ...
    return (
      <div key={document.id} className="document-node  ">
        <div 
          ...
          onClick={() => {
            // If this is a parent with children, automatically open the first child
            if (hasChildren && document.children?.length && document.children.length > 0) {
              // Expand the node if it's not already expanded
              if (!isExpanded) {
                toggleNode(document.id, document);
              } else {
                // If already expanded, just select the first child
                onSelectDocument(document.children[0]);
              }
            } else {
  ...
  return (
    <div className="document-sidebar border-r border-gray-200 dark:border-gray-700 h-full overflow-y-auto fixed w-64 bg-white dark:bg-gray-900 pt-[40px]  ">
      {alert && (
        <AlertMessage 
          type={alert.type} 
