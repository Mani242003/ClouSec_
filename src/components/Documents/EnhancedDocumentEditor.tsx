...
import DocumentLinkHandler from './DocumentLinkHandler';
...
const EnhancedDocumentEditor: React.FC<DocumentEditorProps> = ({ documentId, onDocumentUpdated, onNavigateToDocument }) => {
  ...
  const handleAddBlock = async () => {
    ...
    try {
      ...
      setError(null);
      showAlert('success', 'Content block added successfully!');
    } catch (err) {
      console.error('Failed to add block:', err);
      setError('Failed to add content block. Please try again.');
      showAlert('error', 'Failed to add content block. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateBlock = async (blockId: number, content: string) => {
    ...
    try {
      ...
      // Don't show alert for every keystroke update
    } catch (err) {
  ...
  const handleDeleteBlock = async (blockId: number) => {
    ...
    try {
      ...
      setBlocks(newBlocks);
      setError(null);
      showAlert('success', 'Content block deleted successfully!');
    } catch (err) {
      console.error('Failed to delete block:', err);
      setError('Failed to delete content block. Please try again.');
      showAlert('error', 'Failed to delete content block. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleMoveBlock = async (blockId: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      setError(null);
      showAlert('success', 'Content block moved successfully!');
    } catch (err) {
      console.error('Failed to move block:', err);
      setError('Failed to reorder content blocks. Please try again.');
      showAlert('error', 'Failed to reorder content blocks. Please try again.');
  ...
  return (
    <div className="p-6">
      {/* Document link handler to intercept clicks on document links */}
      <DocumentLinkHandler onNavigateToDocument={handleNavigateToDocument} />
      ...
      {blocks.length === 0 ? (
        ...
        <div className="space-y-6">
          {blocks.map((block) => (
            <div key={block.id} className="border-t border-gray-300 rounded-lg overflow-hidden">
              {isAdmin && (
