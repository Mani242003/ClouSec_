...
export const createDocument = async (title: string, parentId: number | null = null): Promise<Document> => {
...
export const updateDocument = async (id: number, title: string): Promise<Document> => {
  const response = await fetch(`${API_URL}/api/documents/${id}`, {
    method: 'PUT',
    headers: getAuthHeaders(),
...
export const moveDocument = async (id: number, parentId: number | null): Promise<Document> => {
